...
| DB           | Supabase Free, **shared project** (`g_gallery` schema, own role) 🔄 §12.5 | ✅ with hardening          | 500 MB DB (~10× headroom, now shared with another app's data), 5 GB egress untouched (images never touch Supabase), pgvector available ([pricing](https://supabase.com/pricing))                                                                                                                                                                                                                                                                                                                                  |
...
1. **Image-layer privacy is the #1 hole in naive designs**: share-link auth in Next.js does _not_
   protect `cdn.<domain>/…` URLs. Mitigations, in order of adoption:
...
1. **Domain**: `svatebni-fotograf-cechy.cz`, registered at Wedos.cz. The apex/`www` already belong
   to the `svatebni-fotograf-cechy-20` marketing site (its own Vercel project) — g-gallery cannot
   also claim them, so it runs on **`photos.svatebni-fotograf-cechy.cz`** instead (2026-08-21,
   discovered when the Vercel project for g-gallery was created and the apex was already taken).
   ...
   Cloudflare zone: apex/`www` untouched (still the marketing site's), `photos` → Vercel (per Vercel
   docs prefer **DNS-only/grey-cloud** records for the app to avoid double-proxying),
   `cdn.svatebni-fotograf-cechy.cz` → R2 bucket custom domain (proxied — required for Cache +
   Transformations).
...
5. **Supabase**: shared the existing `svatebni-fotograf-cechy-20` project (`byhiodbtosmmtuemldqe`)
   instead of the planned dedicated one — g-gallery is isolated in its own schema (`g_gallery`)
   behind a least-privilege role, keeping the 2nd/last free project slot unspent (2026-08-21, full
   setup in `docs/SETUP.md` §6). Trade: shared connection pool/CPU with that project's live
   booking/orders traffic, accepted in exchange for not burning the last free slot. This makes the
   PostgREST keep-alive cron (§9) likely redundant in practice — the marketing site's own traffic
   should already keep the project out of the pause window — but it stays as a second, independent
   signal rather than being removed on an assumption.
